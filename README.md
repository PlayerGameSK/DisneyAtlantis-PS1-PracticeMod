...
## Functionality:
1. Save your Character's + Ulysses barrel's Current Position (**L2 + Triangle**) *[does not work for the level "Truck Escape" nor while driving the Digger in "Cove" though surprisingly seems to work just fine in "Aktirak Flight" despite it also being a vehicle level / autoscroller]*
...
### Updating the ROM
When a new update gets released, make sure you always restart the installation process (above) with a vanilla version of the game, not an already modified one, as that will either not work or have unforeseen consequences!
